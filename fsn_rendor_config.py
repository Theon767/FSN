...
dataset_type='CustomFSNRENDORDataset'
...
test_pipeline = [
    ...
    dict(type='PadMultiViewImage', size_divisor=32),
    dict(type='DefaultFormatBundle3D', class_names=class_names, with_label=False),
    dict(type='FSN_CustomCollect3D', keys=['img', 'gt_occ','rays'])
]
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        nerf_head=dict(
            ...
            radius=39,
            step_size=0.008, ### Sampling interval on each ray
            use_depth_sup=True),
...
checkpoint_config = dict(interval=5,file_client_args=dict(backend='disk'))

#0 is empty occupancy
#label2semantic = {'地毯及垃圾':0,'窗户及镜子': 1,'墙壁':2,'地面':3,'橱柜':4,'桌子':5,'椅子':6,'垃圾桶':7,'其他':8,'未标注':9}