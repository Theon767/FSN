...
def multiscale_supervision(gt_occ, ratio, gt_shape):
    ...
    for i in range(gt.shape[0]):
        #print('gt_occ[i].shape',gt_occ[i].shape)
        coords = gt_occ[i][:, :3].type(torch.long) // ratio # (H, W ,Z)

        #print('coords', coords)
        # print('gt_shape', gt.shape)
        #print('gt_occ[i][:,3]',gt_occ[i][:,3])
        gt[i, coords[:, 0], coords[:, 1], coords[:, 2]] =  gt_occ[i][:, 3] # (W, H, Z)
    return gt
