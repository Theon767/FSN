...
class FSN(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
                          ...
                          img_metas):
        outs = self.pts_bbox_head(
            pts_feats, img_metas) # Use FSN_head to predcit the occupancy
        loss_inputs = [gt_occ, outs]
        losses = self.pts_bbox_head.loss(*loss_inputs, img_metas=img_metas)
        return losses
    ...
    def forward_train(self,
        ...
        img_feats = self.extract_feat(img=img, img_metas=img_metas) # Extract image features
        losses = dict()
        losses_pts = self.forward_pts_train(img_feats, gt_occ,
                                             img_metas)  # yield loss through difference between gt and prediction
    ...
    def generate_output(self, pred_occ, img_metas):
        ...
        if self.use_semantic:
            ...
            voxel = torch.sigmoid(pred_occ[:, 0])
        for i in range(voxel.shape[0]):
            x = torch.linspace(0, voxel[i].shape[0] - 1, voxel[i].shape[0]) # W
            y = torch.linspace(0, voxel[i].shape[1] - 1, voxel[i].shape[1]) # H
            z = torch.linspace(0, voxel[i].shape[2] - 1, voxel[i].shape[2]) # Z
            X, Y, Z = torch.meshgrid(x, y, z, indexing='ij')
            vv = torch.stack([X, Y, Z], dim=-1).to(voxel.device)
            vertices = vv[voxel[i] > 0.5]
            #Transform to lidar ego coordinates###
            vertices[:, 0] = (vertices[:, 0] + 0.5) * (img_metas[i]['pc_range'][3] - img_metas[i]['pc_range'][0]) /  img_metas[i]['occ_size'][0]  + img_metas[i]['pc_range'][0]
            vertices[:, 1] = (vertices[:, 1] + 0.5) * (img_metas[i]['pc_range'][4] - img_metas[i]['pc_range'][1]) /  img_metas[i]['occ_size'][1]  + img_metas[i]['pc_range'][1]
            vertices[:, 2] = (vertices[:, 2] + 0.5) * (img_metas[i]['pc_range'][5] - img_metas[i]['pc_range'][2]) /  img_metas[i]['occ_size'][2]  + img_metas[i]['pc_range'][2]
            ...
            if self.use_semantic:
                ...
                pcd.colors = o3d.utility.Vector3dVector(color[..., :3])
                vertices = np.concatenate([vertices, semantics[:, None]], axis=-1)
            save_dir=os.path.join('FSN_semantic_visual_dir', img_metas[i]['occ_path'].replace('.npy', '').split('/')[-3])
            os.makedirs(save_dir, exist_ok=True)
            sub_save_dir = os.path.join(save_dir, img_metas[i]['occ_path'].replace('.npy', '').split('/')[-1].split('_')[-1])

            os.makedirs(sub_save_dir, exist_ok=True)


            o3d.io.write_point_cloud(os.path.join(sub_save_dir, 'pred.ply'), pcd)
            np.save(os.path.join(sub_save_dir, 'pred.npy'), vertices)
            for cam_id, cam_path in enumerate(img_metas[i]['filename']):
                os.system('cp {} {}/{}.jpg'.format(cam_path, sub_save_dir, cam_id))
            gt_occ_path=img_metas[i]['occ_path']
            os.system('cp {} {}/gt_occ.npy'.format(gt_occ_path,sub_save_dir))
    

@DETECTORS.register_module()
class FSNRENDOR(MVXTwoStageDetector):
    def __init__(self,
                 use_grid_mask=False,
                 pts_voxel_layer=None,
                 pts_voxel_encoder=None,
    ...
    def forward_pts_train(self,
                          ...
                          img_metas):
        print(img_metas)
        outs,loss_rendering = self.pts_bbox_head(
            pts_feats, img_metas) # Use FSN_head to predcit the occupancy
        print('checkinput',outs.keys())
        print(img_metas)
        losses = self.pts_bbox_head.loss(gt_occ, outs, img_metas=img_metas)
        total_losses={**losses,**loss_rendering}
        return total_losses
    ...
    def forward_test(self, img_metas, img=None, gt_occ=None, **kwargs):
        ...
        output,loss_rendering = self.simple_test(
            img_metas, img, **kwargs)
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        """Test function"""
        outs,loss_rendering = self.pts_bbox_head(x, img_metas)
    ...
    def simple_test(self, img_metas, img=None, rescale=False):
        ...
        bbox_list = [dict() for i in range(len(img_metas))]
        output,loss_rendering = self.simple_test_pts(
            img_feats, img_metas, rescale=rescale)

        return output,loss_rendering
