...
if __name__ == '__main__':
    ...
    cfg = Config.fromfile('test_rendor_config.py')
    dataset = build_dataset(cfg.data.train)
    timestamp = time.strftime('%Y%m%d_%H%M%S', time.localtime())
    log_file = os.path.join(os.path.join(os.getcwd(),'FSN_rendor_logs'), f'{timestamp}.log')
    logger = get_root_logger(
        log_file=log_file, log_level=logging.INFO, name='FSNRENDOR')
    model = build_model(
        cfg.model,
